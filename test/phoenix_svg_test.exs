defmodule PhoenixSVGTest do
  ...
  test "list_files/1" do
    assert {[path1, path2], hash} = PhoenixSVG.list_files(@from)
  ...
  describe "read_file!/1" do
    test "should get an svg with no path" do
      assert {"account", [], svg} = Path.join(@from, "account.svg") |> PhoenixSVG.read_file!(@from)
    ...
    test "should get an svg with a path" do
      assert {"abacus", ["random"], svg} = Path.join(@from, "random/abacus.svg") |> PhoenixSVG.read_file!(@from)
    ...
    test "should raise if svg is not found" do
      assert_raise File.Error, fn -> Path.join(@from, "invalid.svg") |> PhoenixSVG.read_file!(@from) end
    end
  end
